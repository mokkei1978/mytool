...
!  offset = (m-1)*dimx*dimy*dimz*8
offset = 0_MPI_OFFSET_KIND

    call mpi_file_set_view( handle, offset, MPI_BYTE, type_file3d, "native", MPI_INFO_NULL, ierr )
  ...
  offset = (m-1)*isize*jsize*ksize*8

!    call mpi_file_write_all( handle, buffer, isize*jsize*ksize*8, MPI_BYTE, MPI_STATUSES_IGNORE, ierr )
    call mpi_file_write_at_all( handle, offset, buffer, isize*jsize*ksize*8, MPI_BYTE, MPI_STATUSES_IGNORE, ierr )
!    write(6,*) 'write 1:',ipe
